...
public final class MqttDecoder extends ReplayingDecoder<MqttDecoder.DecoderState> {
        ...
        switch (state()) {
            ...
            case READ_VARIABLE_HEADER:
                try {
                    ...
                    if (null != decodedVariableHeader.value)
    ...
    private static Result<?> decodePayload(
        ...
        switch (messageType) {
            case CUSTOMER:
                return decodeCustomerPayload(buffer,  bytesRemainingInVariablePart);
            case CONNECT:
    ...
    private static Result<ByteBuf> decodeCustomerPayload( ByteBuf buffer, int bytesRemainingInVariablePart) {
        byte[] bytes = new byte[bytesRemainingInVariablePart];
        buffer.readBytes(bytes);
        return new Result<>(Unpooled.wrappedBuffer(bytes), bytesRemainingInVariablePart);
    }
